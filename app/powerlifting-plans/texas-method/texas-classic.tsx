...
export default function TexasClassic() {
  ...
  const allExercises = ["深蹲", "卧推", "硬拉", "推举", "力量翻", "力量抓"];

  const getRecommendedExercises = () => {
    if (currentDay === "monday") return ["深蹲", "卧推", "力量翻"];
    if (currentDay === "wednesday") return ["深蹲", "推举", "力量抓"];
  ...
  const handleSaveLog = async () => {
    ...
    const success = await saveLog({
      ...
      notes: `Texas Classic - ${getDayTitle()}${formData.notes ? ` | ${formData.notes}` : ""}`,
    });
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        >
          <Text style={styles.title}>经典3日计划（含奥林匹克举重）</Text>
          ...
          <View style={styles.logForm}>
            ...
            <View style={styles.inputRow}>
              <Text style={styles.label}>动作（推荐）</Text>
              ...
              {/* 其他动作选项 */}
              {recommendedExercises.length < allExercises.length && (
                <>
          ...
          <View style={styles.infoSection}>
            ...
            <Text style={styles.infoSubtitle}>经典3日计划A</Text>
            <Text style={styles.infoText}>
              • 周一：深蹲(周五90%)×5×5, 卧推(周五90%)×5×5, 力量翻5×3
            </Text>
            <Text style={styles.infoText}>
              • 周三：深蹲(周一80%~90%)×5×(2-3), 推举(周一90%~95%)×5×3,
              力量抓(周一90%~95%)×2×(3-4)
            </Text>
            <Text style={styles.infoText}>
              • 周五：深蹲(上周五+2.3kg)×5, 卧推(上周五+0.9kg~1.4kg)×5, 硬拉1×3
            </Text>
          </View>
...
// 样式部分与 texas-powerlifting.tsx 完全相同
